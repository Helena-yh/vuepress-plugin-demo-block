...
const CLASS_HORIZONTAL = 'vuepress-plugin-demo-block__horizontal';
const CLASS_H_CODE = 'vuepress-plugin-demo-block__h_code';
...
const getVueJsTpl = js => {
  const jsContent = js.replace(/export\s+default\s*?\{\n*/, '').replace(/\n*\}\s*$/, '').trim();
  return `new Vue({
  el: '#app',
  ${jsContent}
})`;
...
const getSettings = key => window[SETTINGS_KEY] && window[SETTINGS_KEY][key] !== undefined ? window[SETTINGS_KEY][key] : DEFAULT_SETTINGS[key];
...
const getVanillaDetail = (code, config) => {
  const cssBlock = code.match(/<style>([\s\S]+)<\/style>/);
...
function webController() {
  ...
  nodes.forEach(node => {
    ...
    const detail = type === 'react' ? getReactDetail(code, config) : type === 'vanilla' ? getVanillaDetail(code, config) : getVueDetail(code, config);
    ...
    const horizontalConfig = config.horizontal !== undefined ? config.horizontal : getSettings('horizontal');
